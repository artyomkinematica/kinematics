...
# <<< НОВОЕ: флаги для кросс-камерной проверки >>>
cam1_detection_active = False  # Есть ли детекция на камере 1 в текущем кадре
cam2_detection_active = False  # Есть ли детекция на камере 2 в текущем кадре
cross_cam_mismatch_start = None  # Время начала рассогласования
MISMATCH_TIMEOUT = 0.8  # Секунд задержки перед аварийным поднятием
cross_cam_emergency_triggered = False  # Флаг: аварийное поднятие уже выполнено для текущей детали

cross_cam_mismatch_start_1to2 = None    # Таймер: Cam1 видит, Cam2 нет
cross_cam_emergency_triggered_1to2 = False
...
def auto_control_and_display():
    ...
    # <<< НОВОЕ: добавляем глобальные переменные кросс-камерной проверки >>>
    global cam1_detection_active, cam2_detection_active, cross_cam_mismatch_start, cross_cam_emergency_triggered
    ...
    while True:
        ...
        # Сбрасываем флаги детекции для нового кадра
        cam1_detection_active = False
        cam2_detection_active = False
        ...
        if roi1.size > 0:
            ...
            if merged1:
                # <<< НОВОЕ: детекция на камере 1 есть >>>
                cam1_detection_active = True
                ...
                cv2.putText(frame1, f"Area: {area1} px", (10, 55),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)

                # >>> НОВОЕ: проверка площади для аварийного поднятия <<<
                if auto_mode_active and not emergency_lift_in_progress:
                    if area1 > MAX_AREA_EMERGENCY_2:
                        print(f"🚨 АВАРИЯ (Cam1): Площадь {area1} > {MAX_AREA_EMERGENCY_2}")
                        cv2.putText(frame1, "AREA EMERGENCY!", (10, 80),
        ...
        if X_THRESH_2 + 1 < w2:
            ...
            if roi2.size > 0:
                ...
                if merged2:
                    # <<< НОВОЕ: детекция на камере 2 есть >>>
                    cam2_detection_active = True
                    ...
                    cv2.putText(frame2, f"Area: {area2} px", (10, 55),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 0), 2)

                    # >>> НОВОЕ: проверка площади для аварийного поднятия <<<
                    if auto_mode_active and not emergency_lift_in_progress:
                        if area2 > MAX_AREA_EMERGENCY_2:
                            print(f"🚨 АВАРИЯ (Cam2): Площадь {area2} > {MAX_AREA_EMERGENCY_2}")
                            cv2.putText(frame2, "AREA EMERGENCY!", (10, 80),
                                        cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                            perform_emergency_lift()
        ...
        # === КРОСС-КАМЕРНАЯ ПРОВЕРКА (ОБЕ СТОРОНЫ) ===
        if (auto_mode_active and not emergency_lift_in_progress):

            # --- Случай 1: Cam2 видит, Cam1 НЕ видит ---
            if cam2_detection_active and not cam1_detection_active:
                if not cross_cam_emergency_triggered:
                    if cross_cam_mismatch_start is None:
                        cross_cam_mismatch_start = time.time()
                        print(f"⚠️ Мismatch (2→1): Cam2=да, Cam1=нет. Отсчёт...")
                    elif (time.time() - cross_cam_mismatch_start) >= MISMATCH_TIMEOUT:
                        print("🚨 КРОСС-КАМЕРНАЯ АВАРИЯ (2→1)")
                        cv2.putText(frame1, "CROSS-CAM EMERGENCY (2->1)!", (10, 150),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        cv2.putText(frame2, "CROSS-CAM EMERGENCY (2->1)!", (10, 150),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        perform_emergency_lift()
                        cross_cam_emergency_triggered = True
            else:
                cross_cam_mismatch_start = None
                cross_cam_emergency_triggered = False

            # --- Случай 2: Cam1 видит, Cam2 НЕ видит ---
            if cam1_detection_active and not cam2_detection_active:
                if not cross_cam_emergency_triggered_1to2:
                    if cross_cam_mismatch_start_1to2 is None:
                        cross_cam_mismatch_start_1to2 = time.time()
                        print(f"⚠️ Мismatch (1→2): Cam1=да, Cam2=нет. Отсчёт...")
                    elif (time.time() - cross_cam_mismatch_start_1to2) >= MISMATCH_TIMEOUT:
                        print("🚨 КРОСС-КАМЕРНАЯ АВАРИЯ (1→2)")
                        cv2.putText(frame1, "CROSS-CAM EMERGENCY (1->2)!", (10, 150),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        cv2.putText(frame2, "CROSS-CAM EMERGENCY (1->2)!", (10, 150),
                                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        perform_emergency_lift()
                        cross_cam_emergency_triggered_1to2 = True
            else:
                cross_cam_mismatch_start_1to2 = None
                cross_cam_emergency_triggered_1to2 = False
...
def toggle_stop_continue():
    ...
    global cross_cam_mismatch_start, cross_cam_emergency_triggered  # <<< НОВОЕ: сброс при остановке
    global cross_cam_mismatch_start_1to2, cross_cam_emergency_triggered_1to2
    if stop_requested:
        ...
        emergency_lift_in_progress = False
        emergency_lift_done = False
        cross_cam_mismatch_start = None  # <<< НОВОЕ
        cross_cam_emergency_triggered = False  # <<< НОВОЕ
        cross_cam_mismatch_start_1to2 = None
        cross_cam_emergency_triggered_1to2 = False
        auto_btn.config(text="Автомат", bg="lightblue")
...
def reset_success_state():
    ...
    global success_zone1_entered, success_zone2_entered, success_partial_time, emergency_lift_done, emergency_lift_in_progress
    global success_full_start_time, success_full_confirmed
    # <<< НОВОЕ: добавляем сброс новых переменных >>>
    global cam1_detection_active, cam2_detection_active, cross_cam_mismatch_start, cross_cam_emergency_triggered
    global cross_cam_mismatch_start_1to2, cross_cam_emergency_triggered_1to2
    ...
    emergency_lift_done = False
    emergency_lift_in_progress = False
    # <<< НОВОЕ: сброс флагов кросс-камерной проверки >>>
    cam1_detection_active = False
    cam2_detection_active = False
    cross_cam_mismatch_start = None
    cross_cam_emergency_triggered = False
